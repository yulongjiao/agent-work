  ...
  const startHold = () => {
    ...
    progressTimer.current = setInterval(() => {
      ...
      if (p >= 100) {
        ...
        launch();
        setLaunched(true);
        setTimeout(() => router.push("/employer/dashboard"), 3000);
      }
    }, 30);
  };
  ...
  if (launched) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-black px-8">
        ...
        <motion.p className="text-[15px] text-white/60 text-center leading-relaxed max-w-[280px]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}>你的招聘助理正在全网搜寻人才。<br />有合适候选人会立即通知你。</motion.p>
        <motion.div className="mt-10 flex items-center gap-2" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1 }}>
          ...
          <span className="text-[13px] text-[#34C759]">正在扫描 12,847 份简历...</span>
        </motion.div>
      </div>
    );
  }
    ...
    <div className="flex flex-col min-h-screen bg-[#FAFAFA] relative">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-8 pt-16 pb-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center w-full">
          ...
          <h2 className="text-[22px] font-semibold text-[#1D1D1F] mb-1.5">招聘助理已就绪</h2>
      ...
      <motion.div className="px-5 pb-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
        <div className="bg-white rounded-2xl border border-black/[0.04] overflow-hidden" style={{ boxShadow: "0 2px 16px rgba(0,0,0,0.04)" }}>
          {[
            { icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#1D1D1F" strokeWidth="1.5"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>, title: "智能筛选", desc: "从海量简历中精准匹配，只推送最合适的候选人" },
            { icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#1D1D1F" strokeWidth="1.5"><path d="M22 3 2 12l9 2 2 9z" /></svg>, title: "主动触达", desc: "代替你与候选人初步沟通，摸清意向和期望" },
            { icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#1D1D1F" strokeWidth="1.5"><path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z" /><path d="m9 12 2 2 4-4" /></svg>, title: "面试协调", desc: "自动安排面试时间和流程，减少反复沟通" },
          ].map((item, i) => (
